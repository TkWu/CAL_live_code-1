...
/**
 * Created by ShihMin on 2015/11/20.
 * Environment: DEV
 */
public abstract class CIWSBaseReqLanuch extends AsyncTask<CIRequest, Void, Void> implements CIWebservice {
    ...
    protected CIRequest           m_request     = null;
    protected CIResponseCallback  m_callback    = null;

    protected int         m_iRespCode   = 0;
    protected String      m_strRespBody = null;
    protected CIResponse  m_response    = null;

    public CIWSBaseReqLanuch() {}

    @Override
    public void connection(CIRequest request, CIResponseCallback callback) {
        this.m_request = request;
        this.m_callback = callback;

        //2016-06-20 modigly by ryan for 由於一啟動APP需要同時發送多支Api 所以將AsyncTask調整為可同時並行, 此設定扔有五個線程的限制
        //execute(request);
    ...
    @Override
    protected Void doInBackground(CIRequest... requests) {
        doConnection(requests[0]);
        return null;
    }

    @Override
    protected void onCancelled(Void e) {
        super.onCancelled(e);
    }

    protected void doConnection( CIRequest requests ){
        ...
        } catch ( NoSuchAlgorithmException | KeyManagementException | ClassCastException e) {
           SLog.e(TAG, "==================================================================");
            exception = e;
            e.printStackTrace();
        } catch ( IOException e ) {
           SLog.e(TAG, "==================================================================");
            m_iRespCode = Integer.valueOf(CIWSResultCode.HTTP_RESPONSE_TIME_OUT);
            exception = e;
            e.printStackTrace();
        }
    ...
    protected void httpConnection(CIRequest request) throws IOException, NoSuchAlgorithmException, KeyManagementException, ClassCastException {
    ...
    protected void connection(HttpURLConnection httpConn, CIRequest request) throws IOException, NoSuchAlgorithmException, KeyManagementException, ClassCastException {
