...
public class CIMyTripsRecyclerForRecordViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    private ArrayList<CIMileageRecord_Info> m_arItemList	= new ArrayList<>();

    //判斷item是否要發光(剛新增的item要發光)
    private boolean                     m_bShadow       = false;

    public CIMyTripsRecyclerForRecordViewAdapter(Context context,
                                                 ArrayList<CIMileageRecord_Info> arDataList,
                                                 boolean bShadow,
    ...
    public void setDatas(ArrayList<CIMileageRecord_Info> datas){
        m_arItemList = datas;
    }
...
//        holder.checkInType = m_arItemList.get(position).getCheckType();
        ...
        //日期
        //2016-07-12 Modify by Kevin for 調整歷史行程使用「里程管理中的 InquiryMileageRecord API來取得」
        holder.tvDate.setText(m_arItemList.get(position).flight_date);
        m_vScaleDef.setTextSize(m_vScaleDef.DEF_TEXT_SIZE_16, holder.tvDate);
        holder.tvGoto.setVisibility(View.INVISIBLE);
    ...
    private void prepareFlight(final ItemHolder holder, final int position){
        ...
        final CIMileageRecord_Info mileageRecordInfo = m_arItemList.get(position);
        ...
        // 2017-7-12 CR 調整使用flight_number 取得航空公司名稱對應
        String Airlines_Name = "";
        if (!TextUtils.isEmpty(mileageRecordInfo.flight_number)) {
            //判斷flight number來顯示航空公司名稱
            if(mileageRecordInfo.flight_number.contains("CI")) {
                Airlines_Name = m_context.getString(R.string.my_trip_ci);
            } else if (mileageRecordInfo.flight_number.contains("AE")){
                Airlines_Name = m_context.getString(R.string.my_trip_ae);
            }
        } else {
            mileageRecordInfo.flight_number = "";
        }
        if(!TextUtils.isEmpty(Airlines_Name) || !TextUtils.isEmpty(mileageRecordInfo.flight_number)) {
            flightHolder.tvAirliner.setText(
                    Airlines_Name + " - " + mileageRecordInfo.flight_number);
        } else {
            flightHolder.tvAirliner.setText("");
        }
        ...
        //2017-7-12 CR paser flight_desc 取得起飛及抵達地
        if(!TextUtils.isEmpty(mileageRecordInfo.flight_desc) && mileageRecordInfo.flight_desc.contains("-")){
            String[] stations = mileageRecordInfo.flight_desc.split("-");
            Departure_Station_Name = stations[0].trim();
            Arrival_Station_Name = stations[1].trim();
        } else {
        ...
        flightHolder.tvFromLocation.setText(Departure_Station_Name);
        ...
        flightHolder.tvToLocation.setText(Arrival_Station_Name);
...
//            itineraryInfo.Departure_Time = "";
        ...
        //TODO 2017-7-12 CR 來源沒有暫時不顯示
        flightHolder.tvFromTime.setVisibility(View.INVISIBLE);
        flightHolder.ivFromTime.setVisibility(View.INVISIBLE);
...
//            itineraryInfo.Arrival_Time = "";
        ...
        //TODO 2017-7-12 CR 來源沒有暫時不顯示
        flightHolder.tvToTime.setVisibility(View.INVISIBLE);
        flightHolder.ivToTime.setVisibility(View.INVISIBLE);
        flightHolder.rl_arrow.setVisibility(View.INVISIBLE);
...
//            shadowLayout = (ShadowLayout) itemLayoutView.findViewById(R.id.shadowlayout);
    ...
    private static class FlightHolder extends RecyclerView.ViewHolder {
        ...
        private FlightHolder(View flightLayoutView) {
            ...
            rl_flight_from_to = (RelativeLayout)flightLayoutView.findViewById(R.id.rl_flight_from_to);
            rl_arrow = (RelativeLayout)flightLayoutView.findViewById(R.id.rl_arrow);
