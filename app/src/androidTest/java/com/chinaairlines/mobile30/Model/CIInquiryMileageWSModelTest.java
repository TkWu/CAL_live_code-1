 ...
 */
public class CIInquiryMileageWSModelTest extends SBaseAndroidTestCase {

    /**
     * 功能：查詢會員累積里程數。向API請求資料後，依據結果調用正確之callback。
     */
    ...
    /**
     * 案例說明：有網路狀態，傳入正確的參數，使用setMileageReq()和DoConnection()向WS請求會員累積里程數成功
     * 輸入參數：m_reqMileage
        ...
        CIInquiryMileageModel model = new CIInquiryMileageModel(
                new CIInquiryMileageModel.InquiryMileageCallBack() {
                    ...
                    public void onInquiryMileageSuccess(String rt_code,
                        ...
                        //返回卡號與請求卡號一致
                        Assert.assertEquals( Config.CARD_NO, mileageResp.card_no );

                        //總里程數必須大於等於0
                        Assert.assertTrue( 0 <= Integer.valueOf(mileageResp.mileage) );
    ...
    /**
     * 案例說明：無網路狀態，傳入正確的參數，使用setMileageReq()和DoConnection()向WS請求會員累積里程數失敗
     * 輸入參數：m_reqMileage
    ...
    /**
     * 案例說明：有網路狀態，傳入錯誤的參數，使用setMileageReq()和DoConnection()向WS請求會員累積里程數失敗
